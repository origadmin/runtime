...
// Package security provides the core components for the security security framework.
package security
...
// Provider is a thread-safe implementation of security.PolicyProvider.
// It holds registered policy factories and created policy instances.
type Provider struct {
	mu        sync.RWMutex
	factories map[string]security.SecurityFactory
...
func NewProvider() *Provider {
	return &Provider{
		factories: make(map[string]security.SecurityFactory),
...
func (p *Provider) RegisterFactory(name string, factory security.SecurityFactory) {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *Provider) GetPolicy(name string) (security.SecurityPolicy, error) {
	p.mu.RLock()
