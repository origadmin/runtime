...
var (
	// 定义所有支持的中间件类型
	supportedMiddlewareTypes = map[string]struct{}{
		"metadata":        {},
		"logging":         {},
		"selector":        {},
		"rate_limiter":    {},
...
func TestMiddleware_EdgeCases(t *testing.T) {
	...
	t.Run("NilConfig", func(t *testing.T) {
		// 测试nil配置
		clientMWs := middleware.BuildClient(nil)
		assert.Empty(t, clientMWs, "BuildClient with nil config should return empty middlewares")

		serverMWs := middleware.BuildServer(nil)
		assert.Empty(t, serverMWs, "BuildServer with nil config should return empty middlewares")
	})
}
