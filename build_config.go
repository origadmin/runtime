...
type (
	...
	ConfigBuilder interface {
		...
		NewConfig(*configv1.SourceConfig, *config.RuntimeConfig) (config.Config, error)
	}

	// ConfigSyncer is an interface that defines a method for synchronizing a config.
	ConfigSyncer interface {
		SyncConfig(*configv1.SourceConfig, any, *config.RuntimeConfig) error
	}
)

// ConfigBuildFunc is a function type that takes a SourceConfig and a list of Options and returns a Selector and an error.
type ConfigBuildFunc func(*configv1.SourceConfig, *config.RuntimeConfig) (config.Config, error)

// NewConfig is a method that implements the ConfigBuilder interface for ConfigBuildFunc.
func (fn ConfigBuildFunc) NewConfig(cfg *configv1.SourceConfig, rc *config.RuntimeConfig) (config.Config, error) {
...
func (b *builder) NewConfig(cfg *configv1.SourceConfig, rc *config.RuntimeConfig) (config.Config, error) {
	...
	return configBuilder.NewConfig(cfg, rc)
}

// ConfigSyncFunc is a function type that takes a SourceConfig and a list of Options and returns an error.
type ConfigSyncFunc func(*configv1.SourceConfig, any, *config.RuntimeConfig) error

// SyncConfig is a method that implements the ConfigSyncer interface for ConfigSyncFunc.
func (fn ConfigSyncFunc) SyncConfig(cfg *configv1.SourceConfig, v any, rc *config.RuntimeConfig) error {
	// Call the function with the given SourceConfig and a list of Options.
	return fn(cfg, v, rc)
}

// SyncConfig is a method that implements the ConfigSyncer interface for ConfigSyncFunc.
func (b *builder) SyncConfig(cfg *configv1.SourceConfig, v any, rc *config.RuntimeConfig) error {
	b.syncMux.RLock()
	...
	return configSyncer.SyncConfig(cfg, v, rc)
}
